...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, 'public/uploads'); // Specify the destination directory for storing uploads
  },
...
router.post("/admin/books/add", middleware.ensureAdminLoggedIn, upload.single('image'), async (req, res) => {
  try {
    ...
    // Assuming you have a 'imageUrl' field in your Book model to store the image filename
...
// router.post("/admin/books/add", upload.fields([{ name: 'image', maxCount: 1 }, { name: 'pdf', maxCount: 1 }]), async (req, res) => {
